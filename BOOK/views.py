...
def note_add_book(request):
    if request.method == 'POST':
        ...
        comment_text = data['note_book_textarea']
...
def add_new_book(request):
    if request.method == 'GET':
        ...
        book_id_search = request.GET.get('book_id_search', None)
        print(request.GET.keys()[0])
        print('A'*100)

        # title THAT GOOGLE API IS BASED UPON
        title_google = request.GET.get('title_google', None)
        ...
        try:
            ...
            image_url_goodreads = ''
        try:
            category_goodreads = (root_info_get[1][28][0].attrib['name'] + ' ' +
        ...
        if title_goodreads != '' and title_google != '':
            try:
        ...
        elif title_goodreads == '' and title_google != '':
            try:
                book = Book.objects.get(title=title_goodreads)
            except ObjectDoesNotExist:
                book = Book(
            ...
            context = {
                ...
                'title_google': title_google,
                'author_google': author_google,
                'img_url_google': image_url_google,
            }
        else:
            messages.error(request, 'We could not find the requested book. '
